...
class LocalDifferentialPrivacy:
    ...
    def apply_local_dp(self, column: pd.Series, sensitivity_dict: Dict[str, float]) -> List[Union[str, float, int]]:
        ...
        - noisy_result: The column with local differential 
                        privacy applied.
        """
        if column.name.upper() in ['ID', 'WTS_M']: # to not apply noise to ID or weights
            return column
...
def main():
    ...
    privacy = LocalDifferentialPrivacy(args.epsilon)
    
    sensitivity_dict = privacy.calculate_sensitivity(df, args.query_type)
    
    try:
        df_transformed = privacy.apply_to_dataframe(df, sensitivity_dict)
    except Exception as e:
